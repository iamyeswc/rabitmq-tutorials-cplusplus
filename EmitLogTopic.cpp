...
int main(int argc, char *argv[]){
    ...
    while(1){
        char message[26] = {'\0'};
        string topic;
        cout<<"input the topic of logs:"<<endl;
        getline(cin, topic);
        ...
        amqp_basic_publish(connState, 1, amqp_cstring_bytes(exchange.c_str()), amqp_cstring_bytes(topic.c_str()), false, false, nullptr, message_byte);
        cout<<"severity:"<<topic<<"  message:"<<message_byte.bytes<<endl;
