...
int main(int argc, char *argv[]){
    ...
    while(1){
        char message[26] = {'\0'};
        string severity;
        cout<<"input the logs severity:"<<endl;
        getline(cin, severity);
        
        cout<<"input the logs:"<<endl;
        cin.getline(message, sizeof(message));
        amqp_bytes_t message_byte;
        message_byte.len = sizeof(message);
        message_byte.bytes = message;
        amqp_basic_publish(connState, 1, amqp_cstring_bytes(exchange.c_str()), amqp_cstring_bytes(severity.c_str()), false, false, nullptr, message_byte);
        cout<<"severity:"<<severity<<"  message:"<<message_byte.bytes<<endl;
